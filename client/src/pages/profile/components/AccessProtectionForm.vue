<template>
  <div class="settings-page">
    <div class="settings-card">
      <AsyncState v-if="settingsLoading || settingsError" :loading="settingsLoading" :error="settingsError" :show-retry="Boolean(settingsError)" @retry="loadSettings" />
      <template v-else>
        <section class="protection-row">
          <div class="protection-copy">
            <div class="protection-title-row">
              ...
              <span class="protection-status" :class="{ enabled: enabled }">{{ enabled ? t('auth.enabled') : t('auth.disabled') }}</span>
            </div>
            <p>{{ t('auth.accessProtectionDescription') }}</p>
          </div>
          <el-switch v-model="enabled" :disabled="auth.loading" @change="syncProtectionPreview" />
        </section>
        <el-divider />
...
.settings-page { width: 100%; margin: 0; text-align: left; }
.settings-card { position: relative; width: 100%; min-height: 100%; box-sizing: border-box; padding: 24px; text-align: left; }
.settings-card :deep(.async-state) { min-height: 280px; }
.protection-row { display: flex; align-items: center; justify-content: space-between; min-height: 64px; box-sizing: border-box; gap: 16px; }
.protection-copy { min-width: 0; }
.protection-title-row { display: flex; align-items: center; gap: 10px; }
.protection-copy p { margin: 5px 0 0; color: var(--text-secondary); font-size: 13px; }
.protection-label { color: var(--text-primary); font-size: 16px; font-weight: 600; }
.protection-status { padding: 3px 8px; border-radius: 4px; color: var(--text-secondary); background: var(--app-bg); font-size: 12px; }
.protection-status.enabled { color: var(--success-color); background: rgba(103, 194, 58, 0.12); }
.password-section { max-width: 460px; }
.section-heading { margin-bottom: 16px; }
.section-heading h3 { margin: 0; color: var(--text-primary); font-size: 16px; }
.password-form { width: 440px; max-width: 100%; }
.password-form :deep(.el-form-item) { margin-bottom: 16px; }
.settings-error { max-width: 460px; margin-top: 4px; }
